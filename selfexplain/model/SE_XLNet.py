...
class SEXLNet(LightningModule):
    def __init__(self, hparams):
        ...
        self.multihead_attention = torch.nn.MultiheadAttention(
            config.d_model, dropout=0.2, num_heads=8
        )
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument(
            "--lr", default=2e-5, type=float, help="Initial learning rate."
        )
        parser.add_argument(
    ...
    def forward(self, batch):
        ...
        if self.hparams.model_name == "xlnet-base-cased":
            sentence_cls, hidden_state = self.forward_classifier(
                input_ids=tokens, token_type_ids=tokens_mask, attention_mask=tokens_mask
            )
        else:
            sentence_cls, hidden_state = self.forward_classifier(
        ...
        lil_logits = self.lil(
            hidden_state=hidden_state, nt_idx_matrix=padded_ndx_tensor
        )
        ...
        return logits, acc, {"topk_indices": topk_indices, "lil_logits": lil_logits}
    ...
    def gil(self, pooled_input):
        ...
        concat_pooled_concepts = torch.cat(
            [pooled_input.unsqueeze(1), topk_concepts], dim=1
        )
