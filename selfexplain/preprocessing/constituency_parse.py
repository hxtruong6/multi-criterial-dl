...
class ParseTree:
    ...
    def traverse_and_store(self, tree: ParentedTree, parse_tree_stored: List[Dict]):
        label = tree.label()
        ...
        for subtree in tree:
            if type(subtree) == ParentedTree:
                self.traverse_and_store(
                    tree=subtree, parse_tree_stored=parse_tree_stored
                )
    ...
    def get_parse_tree_for_raw_sent(self, raw_sent):
        ...
        parsed_tree_as_list = self.get_one_hot_encoded_vector(
            parse_tree_list=parsed_tree_as_list, num_tokens=num_tokens
        )
